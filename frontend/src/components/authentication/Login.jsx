...
const Login = () => {
  ...
  const handleRegister = (e) => {
    ...
    axios.post("http://localhost:7000/auth/login", user).then((res) => {
      toast.success(res.data.message);
      Cookie.set(import.meta.env.VITE_COOKIE_TOKEN_NAME, res.data.token, { expires: 7 });
      navigate("/");
      setSpin(false);
    }).catch((error) => {
  ...
  return (
    <div className="flex md:flex-row w-full min-h-screen xl:h-screen">
      {/* Form Section */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-7 min-h-screen bg-white font-[Poppins]">
        <form onSubmit={handleRegister} className="w-full max-w-md">
          <h2 className="text-[28px] font-[400] mb-2 font-[BelfastGrotesk]">Get Started Now</h2>
          ...
          <p className="mt-3 text-sm">Don't Have an account ? <span onClick={() => navigate('/register')} className="text-[#018D43] cursor-pointer">Sign Up</span></p>
        </form>
      </div>
